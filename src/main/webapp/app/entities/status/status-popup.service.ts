        ...
        return new Promise<NgbModalRef>((resolve, reject) => {
            ...
            if (id) {
                this.statusService.find(id).subscribe((status) => {
                    status.createdDate = this.datePipe
                        .transform(status.createdDate, 'yyyy-MM-ddTHH:mm:ss');
                    status.lastModifiedDate = this.datePipe
                        .transform(status.lastModifiedDate, 'yyyy-MM-ddTHH:mm:ss');
