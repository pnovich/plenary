...
public class StreetResource {
    ...
    public ResponseEntity<StreetDTO> createStreet(@Valid @RequestBody StreetDTO streetDTO) throws URISyntaxException {
        ...
        if (streetDTO.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new street cannot already have an ID")).body(null);
        }
    ...
    @GetMapping("/streets")
    @Timed
    public ResponseEntity<List<StreetDTO>> getAllStreets(@ApiParam Pageable pageable) {
    ...
    public ResponseEntity<StreetDTO> getStreet(@PathVariable Long id) {
        ...
        StreetDTO streetDTO = streetService.findOne(id);
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(streetDTO));
    }
    ...
    @Timed
    public ResponseEntity<List<StreetDTO>> searchStreets(@RequestParam String query, @ApiParam Pageable pageable) {
